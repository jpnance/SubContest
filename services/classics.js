...
module.exports.pick = function(request, response) {
	...
	Game.findById(request.params.gameId).then(function(game) {
		...
		existingWeekPicksQuery['picks.' + session.username] = { '$exists': true };

		Game.find(existingWeekPicksQuery).then(function(existingWeekPicks) {
